...
def test_orbitiq_benchmark_generation_and_audit(tmp_path):
    ...
    df = generate_orbitiq_benchmark(data_dir=DATA_DIR, output_path=benchmark_path)
    assert len(df) == 2304
...
def test_orbitiq_end_to_end_pipeline_fast(tmp_path):
    ...
    benchmark_path = tmp_path / 'test_orbitiq_benchmark.csv'
    report = run_pipeline(raw_data_dir=DATA_DIR, benchmark_path=benchmark_path, output_dir=output_dir, epochs=1, batch_size=32, lr=0.001, seed=42, skip_plots=True)
