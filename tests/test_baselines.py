...
import numpy as np
import pandas as pd
from scripts.evaluate_baselines import main as baseline_cli_main
...
class BaselineForecastTests(unittest.TestCase):
    ...
    def test_zero_and_persistence_preserve_unbatched_shape(self):
        ...
        np.testing.assert_array_equal(persistence_forecast(history, 3), np.array([[2.0, 20.0], [2.0, 20.0], [2.0, 20.0]]))
    ...
    def test_batched_generation_and_unknown_name(self):
        ...
        forecasts = generate_baseline_forecasts(history, 3, baselines=['zero', 'persistence', 'seasonal', 'drift'], season_length=2)
    ...
    def test_gaussian_process_baseline_is_available_but_not_default(self):
        history = np.stack([np.sin(np.linspace(0, np.pi, 12)), np.linspace(0, 1, 12)], axis=-1)
        prediction = gaussian_process_forecast(history, horizon=3)
        self.assertEqual(prediction.shape, (3, 2))
        self.assertTrue(np.isfinite(prediction).all())
        self.assertNotIn('gaussian_process', generate_baseline_forecasts(history, 2, season_length=4))
    ...
    def test_evaluate_baselines_honours_label_mask(self):
        ...
        mask[0, 0, 3] = False
        reports = evaluate_baselines(history, actual, TARGETS, baselines=['zero', 'persistence'], horizons={'two': 2}, satellite_ids=['G01'], valid_mask=mask)
        self.assertEqual(reports['zero']['all_forecast_points']['per_target']['Error_Clock']['count'], 1)
        self.assertEqual(reports['persistence']['all_forecast_points']['per_target']['Error_X']['mae'], 0.0)
...
class BaselineCliTests(unittest.TestCase):
    ...
    def test_failed_promotion_is_reported_without_nonzero_exit_by_default(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            frame = pd.DataFrame({'Timestamp': timestamps, 'Satellite_ID': 'G01', 'Constellation': 'G', 'Error_X': np.arange(6.0), 'Error_Y': 0.0, 'Error_Z': 0.0, 'Error_Clock': np.arange(6.0) * 1e-09})
            data_path = root / 'data.csv'
            output_path = root / 'report.json'
            prediction_path = root / 'candidate.npy'
            frame.to_csv(data_path, index=False)
            np.save(prediction_path, np.zeros((1, 2, 4)))
            exit_code = baseline_cli_main(['--data', str(data_path), '--horizon', '2', '--lookback', '2', '--baselines', 'zero', '--candidate-predictions', str(prediction_path), '--output', str(output_path)])
