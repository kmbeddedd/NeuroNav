...
def test_orbitiq_preprocessing():
    df = preprocess_data(str(DATA_DIR / 'DATA_GEO_Train.csv'))
    assert len(df) > 50
...
def test_orbitiq_evaluation_pipeline():
    res = evaluate_orbitiq_dataset(data_path=str(DATA_DIR / 'DATA_GEO_Train.csv'), orbit_type='GEO', models_dir='models/orbitiq_pretrained', seq_length=7, test_size=0.2)
