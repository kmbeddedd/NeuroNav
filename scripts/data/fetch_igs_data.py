...
import urllib.error
MIRRORS = {'bkg': 'https://igs.bkg.bund.de/root_ftp/IGS', 'ign': 'https://geodesie.ign.fr/data/pub/igs', 'whu': 'ftp://igs.gnssm.cn/pub/gnss/data'}
...
def download_file(url: str, output_path: Path, timeout: int=30) -> bool:
    ...
    req = urllib.request.Request(url, headers={'User-Agent': 'Satellite-ML-Data-Acquisition/1.0'})
    try:
...
def fetch_multi_gnss_products(target_date: datetime, out_dir: Path, agency: str='WUM', prefer_mirror: str='bkg') -> dict[str, Path | None]:
