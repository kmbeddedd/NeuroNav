...
ORBIT_PROPERTIES = {'GEO01': {'constellation': 'GEO', 'semi_major_axis_m': 42164000.0, 'inclination_rad': np.deg2rad(5.0), 'nominal_clock_drift_s_per_s': 1.2e-14}, 'MEO01': {'constellation': 'MEO', 'semi_major_axis_m': 26559700.0, 'inclination_rad': np.deg2rad(55.0), 'nominal_clock_drift_s_per_s': 3.5e-14}, 'MEO02': {'constellation': 'MEO', 'semi_major_axis_m': 25510000.0, 'inclination_rad': np.deg2rad(64.8), 'nominal_clock_drift_s_per_s': 4e-14}}

def load_and_standardize_raw(file_path: Path) -> pd.DataFrame:
    df = pd.read_csv(file_path)
    df.columns = df.columns.str.strip()
    col_map = {'x_error  (m)': 'x_error', 'x_error (m)': 'x_error', 'y_error  (m)': 'y_error', 'y_error (m)': 'y_error', 'z_error  (m)': 'z_error', 'z_error (m)': 'z_error', 'satclockerror  (m)': 'clock_error_m', 'satclockerror (m)': 'clock_error_m'}
    df.rename(columns=col_map, inplace=True)
    df['utc_time'] = pd.to_datetime(df['utc_time'])
    df = df.sort_values('utc_time').drop_duplicates(subset=['utc_time']).reset_index(drop=True)
    return df

def interpolate_to_uniform_grid(df: pd.DataFrame, base_time: pd.Timestamp, total_steps: int=768, cadence_seconds: float=900.0) -> pd.DataFrame:
...
def generate_orbitiq_benchmark(data_dir: Path, output_path: Path, total_days: int=8, start_timestamp: str='2025-09-01 00:00:00') -> pd.DataFrame:
    total_steps = total_days * 96
    base_time = pd.Timestamp(start_timestamp)
    cadence_seconds = 15 * 60.0
    sources = [('GEO01', data_dir / 'DATA_GEO_Train.csv'), ('MEO01', data_dir / 'DATA_MEO_Train.csv'), ('MEO02', data_dir / 'DATA_MEO_Train2.csv')]
    all_frames = []
    for sat_id, csv_file in sources:
        ...
        for i in range(total_steps):
            ...
            bc_clock = float(0.0001 + props['nominal_clock_drift_s_per_s'] * t_sec)
            ...
            sat_rows.append({'Timestamp': timestamp.strftime('%Y-%m-%d %H:%M:%S'), 'Satellite_ID': sat_id, 'Constellation': props['constellation'], 'Broadcast_X': x_ecef, 'Broadcast_Y': y_ecef, 'Broadcast_Z': z_ecef, 'Broadcast_Clock': bc_clock, 'Modelled_X': mod_x, 'Modelled_Y': mod_y, 'Modelled_Z': mod_z, 'Modelled_Clock': mod_clock, 'Error_X': err_x, 'Error_Y': err_y, 'Error_Z': err_z, '3D_Orbit_Error': derived_3d, 'Error_Clock': err_clock_s})
...
def main():
    ...
    generate_orbitiq_benchmark(data_dir, output_path)
