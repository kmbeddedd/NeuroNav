...
from src.evaluate import compare_candidate_to_baseline, compute_probabilistic_metrics, compute_sample_metrics, compute_tensor_horizon_metrics, evaluate_forecasts
...
from src.visualize import plot_diffusion_samples, plot_frequency_spectrum, plot_multihorizon_heatmap, plot_probabilistic_uncertainty, plot_residual_qq, plot_training_history
...
def parse_args(argv: list[str] | None=None) -> argparse.Namespace:
    ...
    parser.add_argument('--distribution', choices=('student_t', 'gaussian'), default='gaussian')
    parser.add_argument('--student-t-df', type=float, default=3.0)
    parser.add_argument('--orbit-loss-weight', type=float, default=1.0)
    parser.add_argument('--clock-loss-weight', type=float, default=1.0)
    parser.add_argument('--joint-output-head', action='store_true', help='Ablation: use one shared XYZ+clock projection')
    parser.add_argument('--orbit-class-column', default='Orbit_Class')
    ...
    if args.orbit_loss_weight < 0 or args.clock_loss_weight < 0 or args.orbit_loss_weight + args.clock_loss_weight <= 0:
        parser.error('orbit/clock loss weights must be non-negative with a positive sum')
    if args.no_revin:
        args.use_revin = False
    if args.enable_event_head:
        parser.error('No externally sourced maneuver/clock-event labels are present; the old target-threshold pseudo-event task is intentionally disabled.')
    return args

def _gpu_loader(bundle: dict[str, Any], split: str, device: torch.device, batch_size: int, shuffle: bool) -> FastGPUTensorLoader:
    tensors = (torch.as_tensor(bundle[f'X_{split}'], dtype=torch.float32, device=device), torch.as_tensor(bundle[f'Y_{split}'], dtype=torch.float32, device=device), torch.as_tensor(bundle[f'SAT_{split}'], dtype=torch.long, device=device), torch.as_tensor(bundle[f'SPIKE_{split}'], dtype=torch.float32, device=device), torch.as_tensor(bundle[f'TARGET_MASK_{split}'], dtype=torch.float32, device=device))
...
@torch.no_grad()
def validate_epoch(model: GNSSForecaster, loader: FastGPUTensorLoader, use_amp: bool, args: argparse.Namespace) -> tuple[float, float]:
    model.eval()
...
@torch.no_grad()
def _collect_predictions(model: GNSSForecaster, loader: FastGPUTensorLoader, use_amp: bool) -> dict[str, np.ndarray]:
    model.eval()
...
def _inverse_predictions(collected: dict[str, np.ndarray], target_scaler) -> dict[str, np.ndarray]:
    ...
    return {**collected, 'actual_real': np.where(mask, actual, np.nan), 'mean_real': np.where(mask, mean, np.nan), 'scale_real': np.where(mask, scale, np.nan)}

def _model_config(args: argparse.Namespace, bundle: dict[str, Any]) -> dict[str, Any]:
    return {'num_features': bundle['num_features'], 'num_satellites': bundle['num_satellites'], 'd_model': args.d_model, 'bilstm_units': args.bilstm_units, 'gru_units': args.gru_units, 'nhead': args.nhead, 'num_layers': args.num_layers, 'seq_len': SEQ_LEN, 'forecast_horizon': FORECAST_HORIZON, 'output_dim': bundle['output_dim'], 'target_feature_indices': tuple(bundle['target_feature_indices']), 'use_revin': args.use_revin, 'enable_event_head': False, 'separate_orbit_clock_heads': not args.joint_output_head, 'num_orbit_classes': bundle['num_orbit_classes'], 'orbit_class_by_satellite': tuple(bundle['orbit_class_by_satellite']), 'dropout': args.dropout}

def _artifact_metadata(args: argparse.Namespace, bundle: dict[str, Any]) -> dict[str, Any]:
    return {'artifact_schema_version': ARTIFACT_SCHEMA_VERSION, 'model_class': 'GNSSForecaster', 'model_config': _model_config(args, bundle), 'training_config': vars(args), 'target_cols': bundle['target_cols'], 'feature_cols': bundle['feature_cols'], 'target_feature_indices': bundle['target_feature_indices'], 'satellite_classes': bundle['satellite_classes'], 'orbit_class_classes': bundle['orbit_class_classes'], 'orbit_class_column': bundle['orbit_class_column'], 'feature_scaler': scaler_to_state(bundle['feature_scaler']), 'target_scaler': scaler_to_state(bundle['target_scaler']), 'split_metadata': bundle['split_metadata'], 'data_quality_report': bundle['data_quality_report'], 'data_sha256': sha256_file(args.data), 'git_sha': current_git_sha(Path(__file__).resolve().parent), 'runtime': {'python': sys.version, 'platform': platform.platform(), 'numpy': np.__version__, 'torch': torch.__version__}}

def _promotion_rules(target_cols: list[str]) -> list[dict[str, Any]]:
    rules = []
    for label in HORIZON_MAP:
        for target in target_cols:
            rules.append({'metric': f'horizons.{label}.exact_lead.per_target.{target}.mae', 'direction': 'lower'})
        rules.append({'metric': f'horizons.{label}.exact_lead.orbit_3d_vector_error.mae', 'direction': 'lower'})
    return rules

def _calibration_gate(conformal_report: dict[str, Any], required_coverages: tuple[float, ...]=(0.8, 0.9, 0.95), tolerance: float=0.05) -> dict[str, Any]:
    checks = []
    for target, target_report in conformal_report['per_target'].items():
        for coverage in required_coverages:
            ...
            checks.append({'target': target, 'nominal_coverage': coverage, 'empirical_coverage': empirical, 'absolute_error': error, 'passed': error is not None and error <= tolerance})
    return {'passed': bool(checks) and all((check['passed'] for check in checks)), 'maximum_absolute_coverage_error': tolerance, 'checks': checks}

def _run_diffusion(args: argparse.Namespace, forecaster: GNSSForecaster, bundle: dict[str, Any], train_loader: FastGPUTensorLoader, test_loader: FastGPUTensorLoader, device: torch.device, use_amp: bool, output: Path, test_real: dict[str, np.ndarray]) -> dict[str, Any] | None:
    if not args.enable_diffusion:
        return None
    schedule = DiffusionSchedule(steps=DIFFUSION_DEFAULTS['steps'], beta_start=DIFFUSION_DEFAULTS['beta_start'], beta_end=DIFFUSION_DEFAULTS['beta_end'], schedule_type='cosine', device=device)
    denoiser = ConditionalDiffusionDenoiser(context_dim=forecaster.backbone.context_dim, d_model=args.d_model, output_dim=bundle['output_dim']).to(device)
    optimizer = torch.optim.AdamW(denoiser.parameters(), lr=DIFFUSION_DEFAULTS['learning_rate'], weight_decay=DIFFUSION_DEFAULTS['weight_decay'])
    ...
    torch.save({'artifact_schema_version': ARTIFACT_SCHEMA_VERSION, 'model_state_dict': denoiser.state_dict(), 'model_config': {'context_dim': forecaster.backbone.context_dim, 'd_model': args.d_model, 'output_dim': bundle['output_dim'], 'num_layers': 2, 'nhead': 4}, 'schedule': {'steps': schedule.steps, 'type': schedule.schedule_type}, 'target_cols': bundle['target_cols'], 'target_scaler': scaler_to_state(bundle['target_scaler']), 'data_sha256': sha256_file(args.data), 'history': history}, output / 'gnss_diffusion_bundle.pt')
    draws_scaled = []
    ...
    for _ in range(draw_count):
        ...
        for x, _, sat, _, _ in test_loader:
            with torch.no_grad():
                mean, _, _, context = forecaster(x, sat)
                generated = sample_ddim_forecast(denoiser, schedule, context, mean, shape=mean.shape, num_ddim_steps=args.ddim_steps, device=device)
    ...
    sample_report = compute_sample_metrics(test_real['actual_real'], draws_real, bundle['target_cols'], valid_mask=test_real['mask'])
    np.savez_compressed(output / 'diffusion_test_samples.npz', samples=draws_real)
    if not args.skip_plots and len(draws_real):
        plot_diffusion_samples(test_real['actual_real'][0], [draw[0] for draw in draws_real[:10]], str(output / '05_diffusion_samples.png'))
...
def run_training(argv: list[str] | None=None) -> dict[str, Any]:
    ...
    bundle = prepare_pytorch_datasets(args.data, input_window=SEQ_LEN, forecast_horizon=FORECAST_HORIZON, batch_size=args.batch_size, seed=args.seed, orbit_class_column=args.orbit_class_column)
    ...
    best_loss, best_epoch, best_state, best_optimizer_state, patience = (float('inf'), -1, None, None, 0)
    for epoch in range(args.epochs):
        ...
        print(f'Epoch {epoch + 1:03d}/{args.epochs:03d} train={train_loss:.6f} val={val_loss:.6f} scale={scale_mean:.4f}')
        if val_loss < best_loss - 1e-08:
    ...
    torch.save({**metadata, 'model_state_dict': model.state_dict(), 'optimizer_state_dict': best_optimizer_state, 'history': history}, output / 'gnss_hybrid_forecaster_bundle.pt')
    ...
    point_report = evaluate_forecasts(test_real['actual_real'], test_real['mean_real'], bundle['target_cols'], satellite_ids=satellite_ids, valid_mask=test_real['mask'])
    probabilistic_report = compute_probabilistic_metrics(test_real['actual_real'], test_real['mean_real'], test_real['scale_real'], bundle['target_cols'], valid_mask=test_real['mask'], distribution=args.distribution, df=args.student_t_df)
    ...
    history_real = history_scaled * np.asarray(feature_scaler.scale_)[target_indices][None, None, :] + np.asarray(feature_scaler.mean_)[target_indices][None, None, :]
    baseline_predictions = generate_baseline_forecasts(history_real, horizon=FORECAST_HORIZON, season_length=SEQ_LEN)
    ...
    for name, real_prediction in baseline_predictions.items():
        ...
        report = evaluate_forecasts(test_real['actual_real'], real_prediction, bundle['target_cols'], satellite_ids=satellite_ids, valid_mask=test_real['mask'])
        baseline_reports[name] = report
        promotion[name] = compare_candidate_to_baseline(point_report, report, _promotion_rules(bundle['target_cols']), require_all=True)
    quality_report = audit_csv(args.data)
    evaluated_satellites = sorted(set(satellite_ids.tolist()))
    coverage = {'training_vocabulary_satellites': len(bundle['satellite_classes']), 'test_satellites_evaluated': len(evaluated_satellites), 'evaluated_satellite_ids': evaluated_satellites, 'test_samples': int(len(satellite_ids)), 'valid_target_fraction': float(test_real['mask'].mean())}
    diffusion_report = _run_diffusion(args, model, bundle, train_loader, test_loader, device, use_amp, output, test_real)
    final_report = {'model': point_report, 'probabilistic': probabilistic_report, 'conformal': {'calibration': calibration, 'test': conformal_report}, 'calibration_promotion_gate': calibration_gate, 'baselines': baseline_reports, 'promotion_against_each_baseline': promotion, 'promotion_eligible': bool(quality_report['passed'] and calibration_gate['passed'] and promotion and all((item['passed'] for item in promotion.values()))), 'data_quality': quality_report, 'coverage': coverage, 'diffusion': diffusion_report}
    write_json(output / 'evaluation_report.json', final_report)
    np.savez_compressed(output / 'test_predictions.npz', actual=test_real['actual_real'], mean=test_real['mean_real'], scale=test_real['scale_real'], valid_mask=test_real['mask'], satellite_ids=satellite_ids, label_timestamps=bundle['LABEL_TIMESTAMPS_test'])
    if not args.skip_plots:
        plot_training_history(history, str(output / '01_transformer_training_history.png'))
        plot_residual_qq(test_real['actual_real'], test_real['mean_real'], bundle['target_cols'], str(output / '06_residual_qq.png'))
        legacy_horizons = compute_tensor_horizon_metrics(test_real['actual_real'], test_real['mean_real'], bundle['target_cols'], HORIZON_MAP)
        plot_multihorizon_heatmap(legacy_horizons, bundle['target_cols'], str(output / '02_multihorizon_mae_heatmap.png'))
        plot_probabilistic_uncertainty(test_real['actual_real'][0], test_real['mean_real'][0], test_real['scale_real'][0], str(output / '03_probabilistic_uncertainty.png'))
        plot_frequency_spectrum(test_real['actual_real'][0], test_real['mean_real'][0], str(output / '04_frequency_spectrum.png'))
