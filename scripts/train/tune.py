...
def run_tuning(argv: list[str] | None=None) -> optuna.Study:
    ...
    bundle = prepare_pytorch_datasets(args.data, input_window=SEQ_LEN, forecast_horizon=FORECAST_HORIZON, batch_size=64, seed=args.seed)
    train_tensors = TensorDataset(torch.as_tensor(bundle['X_train'], dtype=torch.float32), torch.as_tensor(bundle['Y_train'], dtype=torch.float32), torch.as_tensor(bundle['TARGET_MASK_train'], dtype=torch.float32))
    val_tensors = TensorDataset(torch.as_tensor(bundle['X_val'], dtype=torch.float32), torch.as_tensor(bundle['Y_val'], dtype=torch.float32), torch.as_tensor(bundle['TARGET_MASK_val'], dtype=torch.float32))
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        train_loader = DataLoader(train_tensors, batch_size=batch_size, shuffle=True, generator=generator, pin_memory=device.type == 'cuda')
        val_loader = DataLoader(val_tensors, batch_size=batch_size, shuffle=False, pin_memory=device.type == 'cuda')
        model = BiLSTMGRUPyTorchModel(seq_len=SEQ_LEN, n_features=bundle['num_features'], output_dim=bundle['output_dim'], target_feature_indices=tuple(bundle['target_feature_indices']), forecast_horizon=FORECAST_HORIZON, bilstm_units=bilstm_units, gru_units=gru_units, dropout_1=dropout_1, dropout_2=dropout_2, separate_orbit_clock_heads=True).to(device)
        optimizer = torch.optim.AdamW(model.parameters(), lr=learning_rate, weight_decay=1e-05)
    ...
    payload = {'objective': 'masked validation MAE in standardized target space', 'seed': args.seed, 'split_metadata': bundle['split_metadata'], 'best_value': study.best_value, 'best_params': study.best_params, 'trials': [{'number': trial.number, 'state': trial.state.name, 'value': trial.value, 'params': trial.params} for trial in study.trials]}
