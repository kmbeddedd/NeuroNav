...
DEFAULT_PROMOTION_RULES = {'all_forecast_points.orbit_3d_vector_error.mae': {'direction': 'lower', 'min_relative_improvement': 0.0}, 'all_forecast_points.clock_error.seconds.mae': {'direction': 'lower', 'min_relative_improvement': 0.0}}

def parse_args(argv: Optional[Sequence[str]]=None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Evaluate executable GNSS residual forecasting baselines')
    ...
    parser.add_argument('--lookback', type=int, help='History rows immediately before the labels (default: enough for requested baselines)')
    parser.add_argument('--season-length', type=int, default=96)
    parser.add_argument('--baselines', nargs='+', choices=BASELINE_NAMES, default=list(DEFAULT_BASELINE_NAMES))
    parser.add_argument('--targets', nargs='+', choices=TARGET_COLS_4, default=list(TARGET_COLS_4))
    ...
    parser.add_argument('--candidate-predictions', help='Optional .npy/.npz candidate predictions aligned to the reported satellite order')
    parser.add_argument('--promotion-baseline', choices=BASELINE_NAMES, default='zero', help='Baseline used for the optional promotion comparison')
    parser.add_argument('--promotion-config', help='Optional JSON mapping of metric paths to promotion rules')
    parser.add_argument('--strict-promotion', action='store_true', help='Return exit code 2 when candidate promotion gates fail')
    return parser.parse_args(argv)

def _load_backtest_arrays(data_path: str, target_cols: Sequence[str], satellite_column: str, constellation_column: str, time_column: str, horizon: int, lookback: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray, list[str], list[str], int]:
    ...
    if 'Error_Clock' in target_cols:
        ...
        if not source_found and {'Broadcast_Clock', 'Error_Clock'}.issubset(frame.columns):
            ...
            sentinel |= np.isclose(broadcast - clock_error, SP3_CLOCK_SENTINEL_SECONDS, rtol=0.0, atol=SP3_CLOCK_SENTINEL_ATOL)
            source_found = True
        if not source_found:
            clock_error = pd.to_numeric(frame['Error_Clock'], errors='coerce').to_numpy(float)
            sentinel |= np.isclose(np.abs(clock_error), SP3_CLOCK_SENTINEL_SECONDS, rtol=0.0, atol=SP3_CLOCK_SENTINEL_ATOL)
    ...
    for satellite, satellite_frame in frame.groupby(satellite_column, sort=True):
        ...
        if not np.all(np.diff(timestamps) == EXPECTED_INTERVAL.to_timedelta64()):
            ...
            continue
        values = selected[list(target_cols)].apply(pd.to_numeric, errors='coerce').to_numpy(dtype=np.float64)
        validity = selected[[f'__{target}_valid' for target in target_cols]].to_numpy(dtype=bool)
    ...
    if not satellite_ids:
        raise ValueError(f'no satellite has the required {lookback} history + {horizon} label rows')
    return (np.stack(histories), np.stack(actuals), np.stack(actual_masks), satellite_ids, constellations, skipped_noncontiguous)
...
def _load_predictions(path: str) -> np.ndarray:
    ...
    if isinstance(loaded, np.lib.npyio.NpzFile):
        try:
            ...
            raise ValueError('candidate NPZ must contain predictions/prediction/pred/mu or exactly one array')
        finally:
            loaded.close()
...
def main(argv: Optional[Sequence[str]]=None) -> int:
    ...
    history, actual, actual_mask, satellite_ids, constellations, skipped_noncontiguous = _load_backtest_arrays(args.data, args.targets, args.satellite_column, args.constellation_column, args.time_column, args.horizon, lookback)
    ...
    baseline_reports = evaluate_baselines(history, actual, args.targets, baselines=args.baselines, season_length=args.season_length, horizons=horizons, satellite_ids=satellite_ids, constellations=constellations, valid_mask=actual_mask)
    report: Dict[str, Any] = {'metadata': {'data': str(Path(args.data).resolve()), 'satellites': satellite_ids, 'target_cols': list(args.targets), 'lookback_steps': lookback, 'forecast_steps': args.horizon, 'label_policy': 'final horizon rows per satellite; history strictly precedes labels', 'cadence_contract': 'every scored history+label block has exact 15-minute steps', 'skipped_noncontiguous_satellites': skipped_noncontiguous, 'valid_label_count_by_target': {target: int(actual_mask[..., index].sum()) for index, target in enumerate(args.targets)}}, 'baselines': baseline_reports}
    ...
    if args.candidate_predictions:
        ...
        candidate_report = evaluate_forecasts(actual, candidate_predictions, args.targets, horizons=horizons, satellite_ids=satellite_ids, constellations=constellations, valid_mask=actual_mask)
        if args.promotion_baseline not in baseline_reports:
            raise ValueError(f'promotion baseline {args.promotion_baseline!r} was not among --baselines')
        rules = _load_rules(args.promotion_config, args.targets)
        promotion = compare_candidate_to_baseline(candidate_report, baseline_reports[args.promotion_baseline], rules)
        promotion_failed = not promotion['passed']
        report['candidate'] = candidate_report
        report['promotion'] = {'baseline': args.promotion_baseline, 'rules': rules, **promotion}
        ...
        for comparison in promotion['comparisons']:
            print(f"    {comparison['metric']}: {('PASS' if comparison['passed'] else 'FAIL')}")
    if args.output:
