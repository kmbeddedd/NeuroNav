...
# Fraction of estimated horizon-specific bias to remove.
...
def main():
    ...
    for validation_day in VALIDATION_DAYS:
        ...
        for window_hours in CAL_WINDOWS_HOURS:
            ...
            calibration_horizons = _

            for horizon_lo, horizon_hi in HORIZON_BINS:
                ...
                calibration_resid_bin = (
                    calibration_residuals[calibration_mask]
                )
                ...
                for bias_type, bias in [
                    ...
                    for shrinkage in SHRINKAGES:
                        ...
                        calibrated_pred[target_mask] -= correction
                        ...
                        upper = (
                            "inf"
                            if np.isinf(horizon_hi)
                            else str(int(horizon_hi))
                        )

                        print(
                            f"window={window_hours:>2}h "
                            f"horizon={int(horizon_lo):>4}-{upper:>4} "
                            f"type={bias_type:>6s} "
