...
class PredictionRouter:
    ...
    def predict(
        ...
        target_times: Optional[Sequence[Any]] = None,
    ) -> pd.DataFrame:
        ...
        for sat_id, sat_df in series_by_sat.items():
            ...
            if target_times is not None and len(target_times) > 0:
                forecast_times = pd.DatetimeIndex(target_times)
                steps_count = len(forecast_times)
            else:
            ...
            sat_record: Dict[str, Any] = {
                "forecast_step": np.arange(1, steps_count + 1),
