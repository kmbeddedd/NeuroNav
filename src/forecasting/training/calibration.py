...
def rank_candidates_hierarchically(
    candidate_evaluations: Dict[str, Dict[str, Any]],
    tie_tolerance: float = 1e-4,
) -> List[str]:
    """Ranks candidate model names strictly following the Official Competition Hierarchy with tie tolerance."""
    def _cmp(a_name: str, b_name: str) -> int:
        eval_a = candidate_evaluations[a_name]
        eval_b = candidate_evaluations[b_name]
        code, _ = compare_models_hierarchical(eval_a, eval_b, tie_tolerance=tie_tolerance)
        if code != 0:
            return -code  # code=1 means a is better than b, so a precedes b in sort (-1)
        if a_name < b_name:
            return -1
        elif a_name > b_name:
            return 1
    ...
    return sorted(candidate_evaluations.keys(), key=functools.cmp_to_key(_cmp))
...
class CalibrationPipeline:
    ...
    def run_calibration(
        ...
        for sat_id in common_sats:
            ...
            # Ranks candidates strictly using the official 3-tier hierarchy with tie tolerance
            sorted_candidates = rank_candidates_hierarchically(
                candidate_evaluations, tie_tolerance=self.tie_tolerance
            )
    ...
    def train_single_satellite(
        ...
        Competition Evaluation Hierarchy with tie tolerance. Persists model artifacts to
        ...
        # Resolve explicit physics mode semantics
        if state_df is not None:
            physics_mode = "provided"
            orbit_state_source = "provided"
            state_provider = get_orbital_state_provider("provided", state_df=state_df)
            eff_use_ric = True if use_ric is None else bool(use_ric)
            eff_use_srp = True if use_srp is None else bool(use_srp)
        elif physics_mode == "provided":
            raise ValueError("physics_mode='provided' requires 'state_df' to be supplied.")
        elif physics_mode == "none":
            orbit_state_source = "none"
            state_provider = None
            eff_use_ric = False
            eff_use_srp = False
        else:
            ...
            orbit_state_source = "nominal_approximation"
            state_provider = get_orbital_state_provider("nominal_approximation")
            eff_use_ric = False if use_ric is None else bool(use_ric)
            eff_use_srp = True if use_srp is None else bool(use_srp)
        ...
        for model_name in models_to_eval:
            try:
                ...
                if "random_forest" in model_name:
                    kwargs["use_ric"] = eff_use_ric or ("ric" in model_name)
                    kwargs["enable_srp"] = eff_use_srp or ("srp" in model_name)
                    kwargs["physics_mode"] = physics_mode
                    kwargs["orbital_state_provider"] = state_provider
                elif "harmonic_ridge" in model_name:
                    kwargs["use_ric"] = eff_use_ric or ("ric" in model_name)
                    kwargs["use_srp"] = eff_use_srp or ("srp" in model_name)
                    kwargs["physics_mode"] = physics_mode
                    kwargs["orbital_state_provider"] = state_provider
        ...
        # Official Competition Hierarchy ranking with tie tolerance
        sorted_candidates = rank_candidates_hierarchically(
            candidate_evaluations, tie_tolerance=self.tie_tolerance
        )
        winner_name = sorted_candidates[0]
        ...
        # Optional orbital state artifact persistence
        state_artifact = None
        if state_df is not None:
            state_artifact_path = sat_artifact_dir / "orbital_state.csv"
            state_df.to_csv(state_artifact_path, index=True)
            state_artifact = str(state_artifact_path)
        ...
        cadence_class = dataset.sampling.cadence_classification if dataset.sampling else "regular"
        resample_applied = bool(dataset.sampling and dataset.sampling.interpolated_epochs > 0)
        training_duration = (datetime.now(timezone.utc) - t_start).total_seconds()
        ...
        selection = self.registry.register_calibration_winner(
            ...
            use_ric=eff_use_ric or ("ric" in winner_name),
            use_srp=eff_use_srp or ("srp" in winner_name),
            cadence_minutes=cadence,
            cadence_classification=cadence_class,
            resampling_applied=resample_applied,
        ...
        return {
            ...
            "feature_manifest": feat_dict,
            "physics_mode": physics_mode,
            "orbital_state_source": orbit_state_source,
            "state_artifact": state_artifact,
        }
