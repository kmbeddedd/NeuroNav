"""Hybrid Transformer Forecaster Adapter."""
...
class TransformerNet(nn.Module):
    def __init__(self, history_dim: int = 7, query_dim: int = 10, d_model: int = 48, nhead: int = 4, seq_len: int = 16):
        super().__init__()
        self.input_proj = nn.Linear(history_dim, d_model)
        ...
        self.heads = _SeparateHeads(d_model + query_dim)

    def forward(self, history: torch.Tensor, query: torch.Tensor) -> torch.Tensor:
        h_proj = self.input_proj(history) + self.pos_emb[:, :history.size(1), :]
        encoded = self.encoder(h_proj)
        features = torch.cat((encoded[:, -1], query), dim=-1)
        return self.heads(features)


class TransformerModel(ForecastModel):
    """Transformer sequence forecaster with multi-head self-attention and separate heads."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(name=name, model_type="transformer", version=version)
        ...
        self.network: Optional[TransformerNet] = None
    ...
    def fit(self, train_df: pd.DataFrame, config: Optional[Dict[str, Any]] = None) -> "TransformerModel":
        ...
        if not hists:
            raise ValueError("Insufficient history rows for TransformerModel")
    ...
    def predict(
        ...
        if not self.is_fitted or self.network is None:
            raise ValueError("TransformerModel must be fitted before predict()")
    ...
    def load(cls, path: Union[str, Path]) -> "TransformerModel":
        ...
        model = cls(
            name=payload.get("name", "Transformer"),
            seq_len=payload.get("seq_len", 16),
            d_model=payload.get("d_model", 48),
            nhead=payload.get("nhead", 4),
    ...
    def get_metadata(self) -> ModelMetadata:
        n_params = sum(p.numel() for p in self.network.parameters()) if self.network else 18000
        return ModelMetadata(
            name=self.name,
            model_type=self.model_type,
            version=self.version,
            architecture="Multi-Head Self-Attention Transformer with Decoupled Heads",
            parameters={"seq_len": self.seq_len, "d_model": self.d_model, "nhead": self.nhead},
            ...
            trainable=True,
            description="Transformer architecture modeling global sequence relationships with scaled dot-product attention.",
