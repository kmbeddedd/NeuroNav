...
class SatelliteSelection:
    ...
    cadence_classification: str = "regular"
    ...
    def get_summary(self) -> Dict[str, Any]:
        """Provides a structured configuration and provenance summary for frontend / reporting."""
        feats = self.feature_manifest.get("features", self.physics_features)
        return {
            ...
            "orbital_state_source": self.orbit_state_source,
            "state_artifact": self.state_artifact,
            "ric_enabled": self.use_ric,
            "solar_physics_context": self.use_srp,
            "original_cadence_minutes": self.cadence_minutes,
            "cadence_classification": self.cadence_classification,
            "resampling_applied": self.resampling_applied,
            "feature_version": self.feature_manifest.get("feature_version", "v2"),
            ...
            "priority_1": self.winning_priority_1,
            "priority_2": self.winning_priority_2,
            "priority_3": self.winning_priority_3,
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "SatelliteSelection":
        return cls(
            ...
            physics_mode=data.get("physics_mode", "nominal"),
            ...
            cadence_classification=data.get("cadence_classification", "regular"),
            resampling_applied=bool(data.get("resampling_applied", False)),
            training_duration_seconds=float(data.get("training_duration_seconds", 0.0)),
...
class SatelliteModelRegistry:
    ...
    def get_satellite_summary(self, satellite_id: str) -> Optional[Dict[str, Any]]:
        """Retrieves a comprehensive configuration and provenance summary for a satellite."""
        sel = self.get_selection(satellite_id)
        if sel is None:
            return None
        return sel.get_summary()

    def get_all_satellites(self) -> List[str]:
        """Returns sorted list of registered satellite identifiers."""
        return sorted(list(self.satellites.keys()))

    def get_satellite_entry(self, satellite_id: str) -> Optional[Dict[str, Any]]:
        """Returns dictionary representation of satellite selection."""
        selection = self.satellites.get(satellite_id)
        return selection.to_dict() if selection else None

    def clear(self) -> None:
        """Clears all satellite model entries and saves empty registry."""
        self.satellites = {}
        self.save()
    ...
    def verify_model_availability(self, satellite_id: str) -> tuple[bool, str]:
        ...
        return True, f"Model '{selection.selected_model}' ({selection.model_version}) ready."
