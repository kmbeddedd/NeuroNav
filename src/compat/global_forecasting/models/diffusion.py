...
from src.compat.global_forecasting.models.transformer import PositionalEncoding
from src.compat.global_forecasting.config import DIFFUSION_DEFAULTS, TARGET_COLS_4

class DiffusionSchedule:

    def __init__(self, steps: int=DIFFUSION_DEFAULTS['steps'], beta_start: float=DIFFUSION_DEFAULTS['beta_start'], beta_end: float=DIFFUSION_DEFAULTS['beta_end'], schedule_type: str='cosine', device: str='cpu'):
        self.steps = steps
        if steps < 2:
        ...
        if schedule_type == 'linear':
            ...
            self.betas = (1.0 - alpha_bar[1:] / alpha_bar[:-1]).clamp(1e-05, 0.999)
        else:
            raise ValueError(f'Unknown diffusion schedule: {schedule_type}')
        self.schedule_type = schedule_type
        self.alphas = 1.0 - self.betas
        self.alpha_bars = torch.cumprod(self.alphas, dim=0)
        self.alpha_bars_prev = torch.cat([torch.ones(1, device=device, dtype=self.alpha_bars.dtype), self.alpha_bars[:-1]])
        self.posterior_variance = (self.betas * (1.0 - self.alpha_bars_prev) / (1.0 - self.alpha_bars)).clamp_min(1e-20)
...
class ConditionalDiffusionDenoiser(nn.Module):
    ...
    def __init__(self, context_dim: int=109, d_model: int=64, output_dim: int=len(TARGET_COLS_4), num_layers: int=2, nhead: int=4):
        super().__init__()
...
@torch.no_grad()
def sample_diffusion_forecast(diffusion_model: nn.Module, schedule: DiffusionSchedule, context: torch.Tensor, mu: torch.Tensor, shape: tuple, noise_scale: float=1.0, device: str='cpu') -> torch.Tensor:
    ...
    for step in reversed(range(schedule.steps)):
        ...
        x = 1.0 / torch.sqrt(alpha) * (x - schedule.betas[step] / torch.sqrt(1.0 - alpha_bar) * predicted_noise)
        if step > 0:
            x = x + torch.sqrt(schedule.posterior_variance[step]) * torch.randn_like(x)
    return mu + x

@torch.no_grad()
def sample_ddim_forecast(diffusion_model: nn.Module, schedule: DiffusionSchedule, context: torch.Tensor, mu: torch.Tensor, shape: tuple, num_ddim_steps: int=20, eta: float=0.0, noise_scale: float=1.0, device: str='cpu') -> torch.Tensor:
    diffusion_model.eval()
    B = shape[0]
    num_ddim_steps = max(2, min(int(num_ddim_steps), schedule.steps))
    times = torch.linspace(schedule.steps - 1, 0, num_ddim_steps, device=device).round().long()
    ...
    for index, curr_step in enumerate(times):
        ...
        alpha_bar_prev = schedule.alpha_bars[next_step] if next_step is not None else torch.ones((), device=device, dtype=alpha_bar_curr.dtype)
        pred_x0 = (x_t - torch.sqrt(1.0 - alpha_bar_curr) * predicted_noise) / torch.sqrt(alpha_bar_curr)
        sigma_t = eta * torch.sqrt(torch.clamp((1.0 - alpha_bar_prev) / (1.0 - alpha_bar_curr) * (1.0 - alpha_bar_curr / alpha_bar_prev), min=0.0))
