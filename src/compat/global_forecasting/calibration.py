...
def _finite_scores(actual: np.ndarray, mean: np.ndarray, scale: np.ndarray, mask: np.ndarray | None) -> tuple[np.ndarray, np.ndarray]:
...
def fit_scaled_conformal(actual: np.ndarray, mean: np.ndarray, scale: np.ndarray, mask: np.ndarray | None=None, coverages: Sequence[float]=(0.8, 0.9, 0.95), min_cell_samples: int=20) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {'method': 'scaled split conformal', 'calibration_shape': list(scores.shape), 'min_cell_samples': int(min_cell_samples), 'coverages': {}}
        ...
        for target in range(targets):
            ...
            for lead in range(horizon):
                if counts[lead, target] >= min_cell_samples:
                    multipliers[lead, target] = _conformal_quantile(scores[:, lead, target], coverage)
                else:
                    multipliers[lead, target] = fallback
                    fallback_used[lead, target] = True
        result['coverages'][f'{coverage:.6g}'] = {'coverage': coverage, 'multipliers': multipliers.tolist(), 'counts': counts.tolist(), 'target_fallback_used': fallback_used.tolist()}
    return result

def conformal_interval(mean: np.ndarray, scale: np.ndarray, calibration: dict[str, Any], coverage: float) -> tuple[np.ndarray, np.ndarray]:
...
def evaluate_conformal_intervals(actual: np.ndarray, mean: np.ndarray, scale: np.ndarray, calibration: dict[str, Any], target_cols: Sequence[str], mask: np.ndarray | None=None) -> dict[str, Any]:
    actual = np.asarray(actual, dtype=np.float64)
    ...
    for target_index, target in enumerate(target_cols):
        ...
        for entry in calibration['coverages'].values():
            ...
            valid = valid_base[..., target_index] & np.isfinite(lower[..., target_index]) & np.isfinite(upper[..., target_index])
            if np.any(valid):
                contained = (actual[..., target_index] >= lower[..., target_index]) & (actual[..., target_index] <= upper[..., target_index])
                width = upper[..., target_index] - lower[..., target_index]
                target_report[f'{coverage:.6g}'] = {'nominal_coverage': coverage, 'empirical_coverage': float(np.mean(contained[valid])), 'mean_width': float(np.mean(width[valid])), 'count': int(valid.sum())}
            else:
                target_report[f'{coverage:.6g}'] = {'nominal_coverage': coverage, 'empirical_coverage': None, 'mean_width': None, 'count': 0}
