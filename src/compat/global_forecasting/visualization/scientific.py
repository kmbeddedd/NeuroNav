...
def plot_residual_qq(actual: np.ndarray, predicted: np.ndarray, target_cols: List[str], save_path: str) -> None:
    observed = np.asarray(actual, dtype=np.float64)
    forecast = np.asarray(predicted, dtype=np.float64)
    if observed.shape != forecast.shape or observed.shape[-1] != len(target_cols):
        raise ValueError('actual, predicted, and target_cols must describe the same tensor')
    os.makedirs(os.path.dirname(os.path.abspath(save_path)), exist_ok=True)
    columns = min(2, len(target_cols))
    rows = int(np.ceil(len(target_cols) / columns))
    fig, axes = plt.subplots(rows, columns, figsize=(6.2 * columns, 4.8 * rows), squeeze=False)
    for index, target in enumerate(target_cols):
        axis = axes[index // columns, index % columns]
        residual = (forecast[..., index] - observed[..., index]).reshape(-1)
        residual = residual[np.isfinite(residual)]
        if residual.size >= 3 and np.ptp(residual) > 0:
            stats.probplot(residual, dist='norm', plot=axis)
            axis.get_lines()[0].set_markerfacecolor('#1565C0')
            axis.get_lines()[0].set_markeredgecolor('#1565C0')
            axis.get_lines()[1].set_color('#E53935')
        else:
            axis.text(0.5, 0.5, 'Insufficient non-constant residuals', ha='center', va='center')
        axis.set_title(f'{target} residuals (n={residual.size:,})')
        axis.grid(alpha=0.25)
    for index in range(len(target_cols), rows * columns):
        axes[index // columns, index % columns].set_visible(False)
    fig.suptitle('Residual Gaussian Q-Q Diagnostics', fontsize=13, fontweight='bold')
    fig.tight_layout()
...
def plot_prediction_vs_actual(all_actuals: Dict[str, np.ndarray], all_preds: Dict[str, np.ndarray], sat_ids: List[str], target_cols: List[str], save_path: str, title_prefix: str='Day-8 Prediction vs Actual') -> None:
    os.makedirs(os.path.dirname(os.path.abspath(save_path)), exist_ok=True)
    if not sat_ids:
        return
    ...
    for row, sat in enumerate(sat_ids):
        for col_idx, (col_name, ylabel) in enumerate([(orbit_col, '3D Orbit Error (m)' if orbit_col == '3D_Orbit_Error' else 'Error X (m)'), (clock_col, 'Clock Error (s)')]):
            ax = axes[row][col_idx]
            ...
            ax.grid(alpha=0.3)
            ax.annotate(f'MAE = {mae:.4f}', xy=(0.02, 0.88), xycoords='axes fraction', fontsize=8, bbox=dict(boxstyle='round,pad=0.2', fc='white', alpha=0.7))
...
def plot_multihorizon_heatmap(horizon_results: Dict[str, Dict], target_cols: List[str], save_path: str) -> None:
    os.makedirs(os.path.dirname(os.path.abspath(save_path)), exist_ok=True)
    h_labels = list(horizon_results.keys())
    col_labels = [c.replace('Error_', '').replace('3D_Orbit_', '3D_') for c in target_cols]
    mat = np.array([[horizon_results[h].get(col, horizon_results[h].get(f'{col}_MAE', 0.0)) for col in target_cols] for h in h_labels])
    ...
    for r in range(len(h_labels)):
        for c in range(len(target_cols)):
            ax.text(c, r, f'{mat[r, c]:.4f}', ha='center', va='center', fontsize=8, color='black' if mat[r, c] < mat.max() * 0.6 else 'white')
    plt.tight_layout()
    plt.savefig(save_path, dpi=150, bbox_inches='tight')
    plt.close()
    print(f'  Plot saved: {save_path}')

def plot_residual_distributions(all_actuals: Dict[str, np.ndarray], all_preds: Dict[str, np.ndarray], target_cols: List[str], save_path: str) -> None:
    os.makedirs(os.path.dirname(os.path.abspath(save_path)), exist_ok=True)
    col_labels = [c.replace('Error_', '').replace('3D_Orbit_', '3D_') for c in target_cols]
    ...
    for i, (ax, col) in enumerate(zip(axes, target_cols)):
        resids = np.concatenate([all_actuals[s][:, i] - all_preds[s][:, i] for s in all_preds])
        ...
        if sig > 1e-08:
            ...
            ax.plot(x, np.exp(-0.5 * ((x - mu) / sig) ** 2) / (sig * np.sqrt(2 * np.pi)), color='#E53935', lw=2, label=f'N(μ={mu:.2e}, σ={sig:.2e})')
...
def plot_per_satellite_mae(all_results: Dict[str, Dict], target_cols: List[str], save_path: str) -> None:
    os.makedirs(os.path.dirname(os.path.abspath(save_path)), exist_ok=True)
    ...
    for ax, vals, title, unit in [(axes[0], orbit_maes, f"{orbit_col.replace('_', ' ')} MAE (m)", 'm'), (axes[1], clock_maes, 'Clock Error MAE (s)', 's')]:
        ...
        ax.legend(handles=[Patch(color='#1565C0', label='GPS (G)'), Patch(color='#AD1457', label='GLONASS (R)'), plt.Line2D([0], [0], color='black', ls='--', label=f'Mean = {mean_val:.4f} {unit}')], fontsize=8)
    plt.tight_layout()
    plt.savefig(save_path, dpi=150, bbox_inches='tight')
    plt.close()
    print(f'  Plot saved: {save_path}')

def plot_probabilistic_uncertainty(actual: np.ndarray, mu: np.ndarray, sigma: np.ndarray, save_path: str, feature_idx: int=0, feature_name: str='Error_X') -> None:
    os.makedirs(os.path.dirname(os.path.abspath(save_path)), exist_ok=True)
    ...
    plt.fill_between(np.arange(len(upper)), lower, upper, color='#1565C0', alpha=0.25, label='±1σ Predictive Uncertainty')
...
def plot_frequency_spectrum(actual: np.ndarray, predicted: np.ndarray, save_path: str, feature_idx: int=0, feature_name: str='Error_X') -> None:
    os.makedirs(os.path.dirname(os.path.abspath(save_path)), exist_ok=True)
...
def plot_diffusion_samples(actual: np.ndarray, samples: List[np.ndarray], save_path: str, feature_idx: int=0, feature_name: str='Error_X') -> None:
    os.makedirs(os.path.dirname(os.path.abspath(save_path)), exist_ok=True)
