...
from src.compat.global_forecasting.config import DEFAULT_SEED, FEATURE_COLS_PYTORCH, FORECAST_HORIZON, OUTLIER_THRESHOLD_3D, SEQ_LEN, SPIKE_THRESHOLD, TARGET_COLS_4, TRAIN_END_DATE
...
def cadence_report(df: pd.DataFrame, interval: str | pd.Timedelta=EXPECTED_INTERVAL) -> Dict[str, object]:
    ...
    if timestamps.isna().any():
        raise ValueError(f'Timestamp contains {int(timestamps.isna().sum())} unparsable/null values.')
    ...
    return {'interval_minutes': float(expected / pd.Timedelta(minutes=1)), 'duplicate_epochs': duplicates, 'irregular_steps': int(sum(irregular_by_satellite.values())), 'irregular_by_satellite': irregular_by_satellite}
...
def _read_data_frame(data_path: str) -> Tuple[pd.DataFrame, Dict[str, object]]:
    ...
    if df['Timestamp'].isna().any():
        raise ValueError(f"Timestamp contains {int(df['Timestamp'].isna().sum())} unparsable/null values.")
    ...
    if report['duplicate_epochs']:
        raise ValueError(f"Dataset contains duplicate (Satellite_ID, Timestamp) rows: {report['duplicate_epochs']}. Resolve the point-in-time join first.")
    return (apply_target_validity_contract(df), report)

def load_and_clean_data(data_path: str, outlier_threshold: float=OUTLIER_THRESHOLD_3D, train_end_date: str=TRAIN_END_DATE, strict_cadence: bool=False) -> Tuple[pd.DataFrame, pd.DataFrame, List[str]]:
    print('=' * 65)
    ...
    if strict_cadence and report['irregular_steps']:
        raise ValueError(f"Dataset is not on an exact 15-minute cadence: {report['irregular_by_satellite']}")
    ...
    if train_df.empty or test_df.empty:
        raise ValueError(f'Split at {split_time} produced train={len(train_df)} and test={len(test_df)} rows. Choose a boundary inside the data range.')
...
def engineer_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for column in TARGET_COLS_4:
        if column in result.columns:
            result[f'{column}_roll_mean'] = result.groupby('Satellite_ID', sort=False)[column].transform(lambda values: values.rolling(8, min_periods=1).mean())
    ...
    if 'Broadcast_Clock' in result.columns:
        ...
        result['Broadcast_Clock_Drift'] = broadcast_clock.groupby(result['Satellite_ID'], sort=False).diff() / elapsed
    return result

def _fit_standard_scaler(frame: pd.DataFrame, columns: Sequence[str], context: str) -> StandardScaler:
    ...
    if empty:
        raise ValueError(f'Cannot fit {context} scaler; no finite training observations for {empty}.')
    return StandardScaler().fit(values)

def _keras_validation_start(frame: pd.DataFrame, seq_len: int, horizon: int, val_split: float) -> pd.Timestamp:
    ...
    if len(timestamps) < minimum:
        raise ValueError(f'Insufficient chronological history for disjoint train/validation labels: need {minimum} unique epochs, found {len(timestamps)}.')
...
def scale_datasets_keras(train_df: pd.DataFrame, test_df: pd.DataFrame, target_cols: List[str]=TARGET_COLS_4, val_split: float=0.1, seq_len: int=SEQ_LEN, horizon: int=FORECAST_HORIZON) -> Tuple[pd.DataFrame, pd.DataFrame, StandardScaler]:
    columns = _validate_target_columns(target_cols)
    train, test = (train_df.copy(), test_df.copy())
    val_start = _keras_validation_start(train, seq_len, horizon, val_split)
    scaler = _fit_standard_scaler(train[train['Timestamp'] < val_start], columns, 'target')
...
def make_sliding_sequences(series: np.ndarray, seq_len: int=SEQ_LEN, horizon: int=FORECAST_HORIZON, timestamps: Optional[Sequence]=None, interval: str | pd.Timedelta=EXPECTED_INTERVAL) -> Tuple[np.ndarray, np.ndarray]:
    ...
    if seq_len <= 0 or horizon <= 0 or len(values) < required:
        raise ValueError(f'Insufficient history: need at least {required} rows with positive seq_len/horizon, found {len(values)}.')
    ...
    for start in range(len(values) - required + 1):
        stop = start + required
        if timestamp_values is not None and (not _is_contiguous(timestamp_values[start:stop], expected)):
            continue
    ...
    if not features:
        raise ValueError('No valid sliding windows remain after cadence and label validation.')
    return (np.asarray(features, np.float32), np.asarray(targets, np.float32))

def build_keras_sequences(train_df: pd.DataFrame, complete_sats: List[str], seq_len: int=SEQ_LEN, horizon: int=FORECAST_HORIZON, target_cols: List[str]=TARGET_COLS_4, val_split: float=0.1, seed: int=DEFAULT_SEED, return_metadata: bool=False):
    del seed
    columns = _validate_target_columns(target_cols)
    val_start = _keras_validation_start(train_df, seq_len, horizon, val_split)
    parts = {split: {key: [] for key in ('X', 'Y', 'input_ts', 'label_ts', 'sat')} for split in ('train', 'val')}
    skipped_gap = skipped_invalid = purged = 0
    for satellite_id in complete_sats:
        sat_df = train_df[train_df['Satellite_ID'] == satellite_id].sort_values('Timestamp')
        ...
        for start in range(len(sat_df) - seq_len - horizon + 1):
            ...
            if pd.Timestamp(label_ts[-1]) < val_start:
                ...
                purged += 1
                continue
            parts[split]['X'].append(np.nan_to_num(values[start:start + seq_len], nan=0.0))
    ...
    if not len(X_train) or not len(X_val):
        raise ValueError(f'No usable chronological train/validation samples. Each satellite needs at least {seq_len + 2 * horizon} contiguous epochs and valid labels.')
    print(f'  Training sequences   : {X_train.shape}')
    print(f'  Validation sequences : {X_val.shape}')
    print(f'  Purged windows        : boundary={purged}, gaps={skipped_gap}, invalid_labels={skipped_invalid}')
    if not return_metadata:
        return (X_train, y_train, X_val, y_val)
    metadata = {'validation_start': val_start.isoformat(), 'train_input_timestamps': stack('train', 'input_ts', (seq_len,), 'datetime64[ns]'), 'train_label_timestamps': stack('train', 'label_ts', (horizon,), 'datetime64[ns]'), 'val_input_timestamps': stack('val', 'input_ts', (seq_len,), 'datetime64[ns]'), 'val_label_timestamps': stack('val', 'label_ts', (horizon,), 'datetime64[ns]'), 'train_satellite_ids': np.asarray(parts['train']['sat'], dtype=object), 'val_satellite_ids': np.asarray(parts['val']['sat'], dtype=object), 'purged_boundary_windows': purged, 'skipped_noncontiguous_windows': skipped_gap, 'skipped_invalid_target_windows': skipped_invalid}
    return (X_train, y_train, X_val, y_val, metadata)

def _resolve_split_boundaries(df: pd.DataFrame, input_window: int, forecast_horizon: int, test_size: float, val_ratio: float, train_end_date: str) -> Tuple[pd.Timestamp, pd.Timestamp]:
    ...
    if len(timestamps) < minimum:
        raise ValueError(f'Insufficient history for purged train/validation/test blocks: need {minimum} unique epochs, found {len(timestamps)}.')
    configured = pd.Timestamp(train_end_date)
    if timestamps[0] < configured <= timestamps[-1]:
        test_start = configured
    else:
        test_steps = max(forecast_horizon, int(np.ceil(len(timestamps) * test_size * val_ratio)))
    ...
    if len(test_epochs) < forecast_horizon:
        raise ValueError(f'Test block has {len(test_epochs)} epochs; horizon needs {forecast_horizon}.')
    requested_val = max(forecast_horizon, int(np.ceil(len(timestamps) * test_size * (1.0 - val_ratio))))
...
def prepare_pytorch_datasets(data_path: str, input_window: int=SEQ_LEN, forecast_horizon: int=FORECAST_HORIZON, spike_threshold: float=SPIKE_THRESHOLD, batch_size: int=32, test_size: float=0.3, val_ratio: float=0.5, train_end_date: str=TRAIN_END_DATE, interval: str | pd.Timedelta=EXPECTED_INTERVAL, seed: int=DEFAULT_SEED, feature_cols: Optional[Sequence[str]]=None, include_physical_features: bool=True, orbit_class_column: str='Orbit_Class', evaluation_mode: str='strict_block') -> Dict:
    ...
    if feature_cols is None:
        ...
        if include_physical_features:
            selected_feature_cols.extend((column for column in PHYSICAL_FEATURE_COLS if column in df.columns))
    else:
    ...
    if missing_target_history:
        raise ValueError(f'feature_cols must retain all four historical targets for residual and naive baselines; missing {missing_target_history}.')
    validation_start, test_start = _resolve_split_boundaries(df, input_window, forecast_horizon, test_size, val_ratio, train_end_date)
    known_satellites = [str(satellite_id) for satellite_id, sat_df in df.groupby('Satellite_ID', sort=True) if _has_contiguous_training_window(sat_df, validation_start, input_window, forecast_horizon, expected)]
    if not known_satellites:
        raise ValueError(f'No satellite has contiguous training history before {validation_start}.')
    ...
    orbit_class_classes: List[str] = []
    orbit_class_by_satellite: List[int] = []
    if orbit_class_column in df.columns:
        training_rows = df[df['Timestamp'] < validation_start]
        class_by_satellite: Dict[str, str] = {}
        for satellite_id in sat_encoder.classes_:
            values = training_rows.loc[training_rows['Satellite_ID'] == satellite_id, orbit_class_column].dropna().astype(str).str.strip()
            values = values[values != '']
            unique = sorted(values.unique().tolist())
            if len(unique) != 1:
                raise ValueError(f'{orbit_class_column} must have exactly one training value for satellite {satellite_id!r}; found {unique}')
            class_by_satellite[str(satellite_id)] = unique[0].upper()
        orbit_encoder = LabelEncoder().fit(list(class_by_satellite.values()))
        orbit_class_classes = orbit_encoder.classes_.tolist()
        orbit_class_by_satellite = [int(orbit_encoder.transform([class_by_satellite[str(satellite)]])[0]) for satellite in sat_encoder.classes_]
    scaler_fit_df = df[df['Timestamp'] < validation_start]
    feature_scaler = _fit_standard_scaler(scaler_fit_df, selected_feature_cols, 'feature')
    ...
    row_mask = np.column_stack([df[f'{column}{VALID_SUFFIX}'].to_numpy(bool) for column in target_cols])
    ...
    for satellite_id, sat_df in working.groupby('Satellite_ID', sort=True):
        ...
        for start in range(len(sat_df) - required + 1):
            ...
            spike = ((np.abs(y_seq) > float(spike_threshold)) & label_mask).any(axis=1).astype(np.float32)
    ...
    def pack(split: str) -> Dict[str, np.ndarray]:
        ...
        return {'X': np.asarray(parts[split]['X'], np.float32), 'Y': np.asarray(parts[split]['Y'], np.float32), 'SAT': np.asarray(parts[split]['SAT'], np.int64), 'SPIKE': np.asarray(parts[split]['SPIKE'], np.float32), 'MASK': np.asarray(parts[split]['MASK'], np.float32), 'input_ts': np.asarray(parts[split]['input_ts'], dtype='datetime64[ns]'), 'label_ts': np.asarray(parts[split]['label_ts'], dtype='datetime64[ns]'), 'satellite_id': np.asarray(parts[split]['satellite_id'], dtype=object)}
    packed = {split: pack(split) for split in ('train', 'val', 'test')}
    datasets = {split: GNSSPyTorchDataset(data['X'], data['Y'], data['SAT'], data['SPIKE'], data['MASK']) for split, data in packed.items()}
...
class FastGPUTensorLoader:
    ...
    def __init__(self, tensors: tuple, batch_size: int=128, shuffle: bool=True, device: Optional[torch.device]=None):
        if not tensors:
    ...
    def __iter__(self):
        indices = torch.randperm(self.num_samples, device=self.device) if self.shuffle else torch.arange(self.num_samples, device=self.device)
