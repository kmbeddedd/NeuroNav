...
def scaler_to_state(scaler: StandardScaler) -> dict[str, Any]:
    state: dict[str, Any] = {'class': type(scaler).__name__, 'with_mean': scaler.with_mean, 'with_std': scaler.with_std}
...
def scaler_from_state(state: dict[str, Any]) -> StandardScaler:
    scaler = StandardScaler(with_mean=bool(state.get('with_mean', True)), with_std=bool(state.get('with_std', True)))
