...
class HorizonGRU(nn.Module):
    ...
    def __init__(
        ...
        # Long-term branch: existing 7-day history.
        ...
        # Short-term branch: recent 1-day history.
        ...
        self.head = nn.Sequential(
            nn.Linear(hidden_size + hidden_size + 2 + 4, 32),
            nn.ReLU(),
            nn.Linear(32, 4),
        )
    ...
    def forward(
        ...
        long_packed = nn.utils.rnn.pack_padded_sequence(
            x_long,
            long_lengths.cpu(),
            batch_first=True,
        ...
        _, long_hidden = self.long_gru(
            long_packed
        )

        long_repr = long_hidden[-1]

        short_packed = nn.utils.rnn.pack_padded_sequence(
            x_short,
            short_lengths.cpu(),
            batch_first=True,
            enforce_sorted=False,
        ...
        _, short_hidden = self.short_gru(
            short_packed
        )

        short_repr = short_hidden[-1]
...
def build_training_examples(df):
    ...
    # Use the most recent observations as the short-term branch.
        short_sequence = sequence[
            -SHORT_HISTORY_STEPS:
        ]

        # Keep the full 7-day sequence.
...
def build_validation_examples(
    ...
    for _, row in (
        ...
        # Keep both branches only when both are valid.
        long_sequences.append(sequence)
        short_sequences.append(short_sequence)
        horizons.append(horizon)
        horizon_regimes.append(horizon_regime)
...
def train_model(
    ...
    X_long_tensor = torch.tensor(
        X_long,
        dtype=torch.float32,
        device=DEVICE,
    )

    long_lengths_tensor = torch.tensor(
        long_lengths,
        dtype=torch.long,
        device=DEVICE,
    )

    X_short_tensor = torch.tensor(
        X_short,
        dtype=torch.float32,
        device=DEVICE,
    )

    short_lengths_tensor = torch.tensor(
        short_lengths,
        dtype=torch.long,
        device=DEVICE,
    )
...
def main():
    ...
    for dataset_name, filename in DATASETS.items():
        ...
        for validation_day in days[1:]:
            ...
            if len(train_long_sequences) < 5:
                continue

            if len(val_long_sequences) == 0:
                continue

            if len(train_long_sequences) != len(train_short_sequences):
                raise RuntimeError(
                    "Training long/short sequence count mismatch: "
                    f"{len(train_long_sequences)} vs "
                    f"{len(train_short_sequences)}"
                )

            if len(val_long_sequences) != len(val_short_sequences):
                raise RuntimeError(
                    "Validation long/short sequence count mismatch: "
                    f"{len(val_long_sequences)} vs "
                    f"{len(val_short_sequences)}"
                )

            # Scale both history branches using training data only.
            ...
            train_long_sequences = [
                ...
                for seq in train_long_sequences
            ]

            train_short_sequences = [
                x_scaler.transform(seq)
                for seq in train_short_sequences
            ]

            val_long_sequences = [
                x_scaler.transform(seq)
                for seq in val_long_sequences
            ]

            val_short_sequences = [
                x_scaler.transform(seq)
                for seq in val_short_sequences
            ]
            ...
            X_train_long, train_long_lengths = (
                pad_sequences(train_long_sequences)
            )

            X_train_short, train_short_lengths = (
                pad_sequences(train_short_sequences)
            )

            X_val_long, val_long_lengths = (
                pad_sequences(val_long_sequences)
            )

            X_val_short, val_short_lengths = (
                pad_sequences(val_short_sequences)
            )
            ...
            print(
                f"Train examples: "
                f"{len(train_long_sequences)}"
            )

            print(
                f"Validation examples: "
                f"{len(val_long_sequences)}"
            )
