...
from src.forecasting.pipeline import evaluate_residuals_official_hierarchy
from src.forecasting.registry import SatelliteModelRegistry
from src.forecasting.router import PredictionRouter
from src.forecasting.validation import TARGET_COLS_INTERNAL, load_telemetry_source, validate_dataset
...
def main() -> None:
    ...
    train_val = validate_dataset(ROOT / "data", min_history_rows=8)
    test_val = validate_dataset(ROOT / "data", min_history_rows=1, is_test_dataset=True)
    ...
    for sat, (train_name, test_name) in SAT_FILES.items():
        train_path, test_path = ROOT / "data" / train_name, ROOT / "data" / test_name
