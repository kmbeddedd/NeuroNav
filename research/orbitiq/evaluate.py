...
FEATURE_COLUMNS = ['x_error (m)', 'y_error (m)', 'z_error (m)', 'satclockerror (m)']
...
class KerasLSTMInference:
        ...
        with h5py.File(self.h5_path, 'r') as f:
            ...
            for l in layers:
                ...
                if cls_name in ('LSTM', 'Dense'):
                    name = l['config']['name']
                    self.layers_config.append({'name': name, 'class': cls_name, 'config': l['config']})
                    ds = {}
    ...
    def _lstm_step(self, x_seq: np.ndarray, kernel: np.ndarray, recurrent_kernel: np.ndarray, bias: np.ndarray, return_sequences: bool=True) -> np.ndarray:
        if x_seq.ndim == 2:
    ...
    def predict(self, x: np.ndarray) -> np.ndarray:
        ...
        for layer in self.layers_config:
            ...
            if cls == 'LSTM':
                ...
                curr = self._lstm_step(curr, w['kernel'], w['recurrent_kernel'], w['bias'], return_sequences=ret_seq)
            elif cls == 'Dense':
                if curr.ndim == 3:
                    curr = curr[:, -1, :]
...
def preprocess_data(file_path: str) -> pd.DataFrame:
    ...
    column_mapping = {'x_error  (m)': 'x_error (m)', 'y_error  (m)': 'y_error (m)', 'z_error  (m)': 'z_error (m)', 'satclockerror  (m)': 'satclockerror (m)'}
    ...
    df['total_position_error'] = np.sqrt(df['x_error (m)'] ** 2 + df['y_error (m)'] ** 2 + df['z_error (m)'] ** 2)
...
def evaluate_residual_distribution(residuals: np.ndarray) -> Dict[str, Any]:
    ...
    return {'mae': mae, 'rmse': rmse, 'max_error': max_err, 'std_error': std_err, 'shapiro_w': float(shapiro_stat), 'shapiro_p_value': float(shapiro_p), 'is_normal': bool(shapiro_p > 0.05)}

def evaluate_orbitiq_dataset(data_path: str, orbit_type: str, models_dir: str, seq_length: int=7, test_size: float=0.2) -> Dict[str, Any]:
    ...
    results = {'dataset': Path(data_path).name, 'orbit_type': orbit_type, 'num_epochs': len(df), 'date_start': str(df['utc_time'].min()), 'date_end': str(df['utc_time'].max()), 'lstm_pretrained': {}, 'random_forest': {}, 'day_8_forecast': {}}
    ...
    for target in FEATURE_COLUMNS:
        ...
        if len(X) < 5:
            continue
        X_train, X_test, y_train_scaled, y_test_scaled = train_test_split(X, y_scaled, test_size=test_size, shuffle=False)
    ...
    for target in FEATURE_COLUMNS:
        ...
        X_train, X_test, y_train_scaled, y_test_scaled = train_test_split(X_rf, y_scaled, test_size=test_size, shuffle=False)
...
def print_formatted_summary(all_results: List[Dict[str, Any]]):
    print('\n' + '=' * 90)
    print('ORBITIQ GNSS ERROR MODELLING - EVALUATION BENCHMARK RESULTS')
    print('=' * 90)
...
def generate_markdown_report(all_results: List[Dict[str, Any]], output_path: str):
    lines = ['# OrbitIQ GNSS Orbit & Clock Error Modeling Evaluation Report', '', 'Evaluation results for pre-trained LSTM neural networks and Random Forest models on the benchmark dataset.', '']
    for res in all_results:
        dataset = res['dataset']
        orbit = res['orbit_type']
        n_epochs = res['num_epochs']
...
def main(argv=None):
    ...
    parser.add_argument('--data-dir', type=str, default='data/ps08', help='Directory containing OrbitIQ datasets')
    ...
    datasets_to_run = [(data_dir / 'DATA_GEO_Train.csv', 'GEO'), (data_dir / 'DATA_MEO_Train.csv', 'MEO'), (data_dir / 'DATA_MEO_Train2.csv', 'MEO')]
    all_results = []
    for csv_file, orbit_type in datasets_to_run:
        if csv_file.exists():
            print(f'Evaluating {csv_file.name} ({orbit_type})...')
            res = evaluate_orbitiq_dataset(str(csv_file), orbit_type=orbit_type, models_dir=str(models_dir))
