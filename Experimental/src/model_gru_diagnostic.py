...
def main():
    ...
    for dataset_name, filename in DATASETS.items():
        ...
        for validation_day in days[1:]:
            ...
            for row_index, residual_row in enumerate(residuals):
                residual_records.append({
                    "dataset": dataset_name,
                    "validation_day": validation_day.date(),
                    "row_index": row_index,
                    "x_residual": residual_row[0],
                    "y_residual": residual_row[1],
                    "z_residual": residual_row[2],
                    "clock_residual": residual_row[3],
                    "horizon_min": val_horizons[row_index],
                })
    ...
    residual_df.to_csv(
        output_dir / "horizon_gru_diagnostic_residuals.csv",
        index=False,
    )
