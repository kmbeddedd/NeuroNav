...
class HorizonGRU(nn.Module):
    ...
    def __init__(
        ...
        # Shared temporal representation + forecast horizon.
        self.shared = nn.Sequential(
            nn.Linear(hidden_size + 1, 32),
            nn.ReLU(),
        )

        # Separate output head for each target.
        self.x_head = nn.Linear(32, 1)
        self.y_head = nn.Linear(32, 1)
        self.z_head = nn.Linear(32, 1)
        self.clock_head = nn.Linear(32, 1)
    ...
    def forward(
        ...
        return torch.cat(
            [
                self.x_head(shared_repr),
                self.y_head(shared_repr),
                self.z_head(shared_repr),
                self.clock_head(shared_repr),
            ],
            dim=1,
        )
