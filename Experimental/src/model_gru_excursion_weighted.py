...
def train_model(
    ...
    # Give rare large-error targets more influence while
    # retaining the existing horizon weighting.
    excursion_scale = np.max(
        np.abs(y),
        axis=1,
    )

    excursion_multiplier = np.ones_like(
        excursion_scale,
        dtype=np.float32,
    )
    ...
    excursion_multiplier[
        excursion_scale >= 10.0
    ] = 1.50

    excursion_multiplier[
        excursion_scale >= 20.0
    ] = 2.00
